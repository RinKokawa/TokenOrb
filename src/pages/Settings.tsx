...
export const Settings = ({ refreshInterval, onBack, onRefreshIntervalChange }: SettingsProps) => {
  ...
  const statusLine = (() => {
    switch (status) {
      case 'online':
        return 'MiniMax API responding';
      case 'mock':
        return 'MINIMAX_TOKEN not set; showing mock data';
      case 'unauthorized':
        return 'MINIMAX_TOKEN was rejected (401/403)';
      case 'offline':
  ...
  return (
    <section className="glass-panel flex h-[400px] w-[320px] flex-col overflow-hidden rounded-2xl p-5 text-slate-200">
      <header className="flex items-center gap-3">
        <button
          type="button"
          aria-label="Back to token monitor"
        ...
        <div>
          ...
          <h1 className="light-text mt-1 text-xl font-semibold tracking-tight">Preferences</h1>
        </div>
      </header>

      <div className="mt-7 space-y-5 overflow-y-auto pr-1">
        <div>
          <p className="light-text text-sm font-medium">MiniMax API</p>
          <p className="mt-1 text-[11px] leading-relaxed text-[var(--muted)]">
            Token lives in <code className="text-indigo-300">.env</code> (git-ignored). Set
            <code className="mx-1 text-indigo-300">MINIMAX_TOKEN</code> and
            <code className="mx-1 text-indigo-300">MINIMAX_GROUP_ID</code>, then restart.
          </p>
          <dl className="mt-3 space-y-1 text-[11px]">
            ...
            <div className="flex items-center justify-between">
              <dt className="text-[var(--muted)]">Models</dt>
              <dd className="light-text font-medium">{modelsCount}</dd>
            </div>
            <div className="flex items-center justify-between">
              <dt className="text-[var(--muted)]">Last fetch</dt>
        ...
        <label className="block">
          <span className="light-text text-sm font-medium">Refresh frequency</span>
          <select
        ...
        <fieldset>
          <legend className="light-text text-sm font-medium">Theme</legend>
